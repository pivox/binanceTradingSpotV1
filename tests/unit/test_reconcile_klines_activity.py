...
def test_reconcile_klines_treats_empty_binance_payload_as_failure(
    tmp_path, monkeypatch
):
    db_url = f"sqlite:///{tmp_path / 'reconcile_klines_empty_payload.db'}"
    engine, sf = _session_factory(db_url)
    try:
        with sf() as session:
            session.add(
        ...
        assert out["gap_requests"] == 1
        assert out["jobs_processed"] == 1
        assert out["gaps_filled"] == 0
        assert out["failures"] == 1

        with sf() as session:
            jobs = list(session.scalars(select(BackfillJob)))
            assert len(jobs) == 1
            assert jobs[0].status in {"RETRY_WAIT", "COOLDOWN", "FAILED_TERMINAL"}
            assert jobs[0].last_http_status == 424
    finally:
        engine.dispose()


def test_reconcile_klines_detects_all_symbols_when_env_unset(tmp_path, monkeypatch):
    db_url = f"sqlite:///{tmp_path / 'reconcile_klines_all_symbols.db'}"
    engine, sf = _session_factory(db_url)
    try:
        with sf() as session:
            session.add_all(
        ...
        monkeypatch.setenv("DATABASE_URL", db_url)
        monkeypatch.setenv("SHARD_COUNT", "8")
        monkeypatch.delenv("RECONCILE_SYMBOLS", raising=False)
        ...
        async def _fake_fetch(
            ...
            *,
            ...
            assert timeframe == "1m"
            assert start_time_ms == 60_000
            return act._HttpResult(
                status=200,
                headers={"X-MBX-USED-WEIGHT-1M": "18"},
                payload=[
                    [60_000, "1001.0", "1002.0", "1000.5", "1001.5", "9.0", 119_999],
                ],
            )
