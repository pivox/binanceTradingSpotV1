...
def main() -> None:
    ...
    parser.add_argument("--k-atr", type=float, default=2.0, dest="k_atr",
                        help="ATR multiplier for SL fallback (phase 2 calibration)")
    parser.add_argument("--r-multiple", type=float, default=1.5, dest="r_multiple",
    ...
    print(f"{sep}")
    print(f"  Config  k={config.k_atr}  R={config.r_multiple}  "
          f"SL_buf={config.sl_buffer_pct:.1%}  max_SL={config.max_sl_pct:.1%}")
    print(f"{'─' * 54}")
    print(f"  Trades    total={m.total_trades}  closed={m.closed_trades}  "
          f"open={m.total_trades - m.closed_trades}")
    print(f"  Winrate   {m.winrate:.1%}  "
          f"({'✓' if m.winrate > 0.5 else '✗'} >50%)")
    print(f"  PF        {m.profit_factor:.2f}  "
          f"({'✓' if m.profit_factor > 1.3 else '✗'} >1.3)")
    print(f"  Drawdown  {m.max_drawdown_pct:.1f}%  "
